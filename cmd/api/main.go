...
func main() {
	...
	flag.StringVar(&cfg.DB.DSN, "db-dsn", "postgres://cart:pass@localhost:5432/carts?sslmode=disable&client_encoding=UTF8", "PostgresSQL DSN")
...
func New(log *jsonlog.Logger, grpcPort int, cfg Config, tokenTTL time.Duration, subsClient *crtgrpc.Client) *Application {
	...
	orderService := cart.New(log, db, tokenTTL, subsClient)
